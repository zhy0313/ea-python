...
def initialize(context):
    ...
    # 获取历史数据的长度
    context.user_data.dmi_window = 30
...
def handle_data(context):
    # 获取历史数据, 取后dmi_window根bar
    hist = context.data.get_price(context.security, count=context.user_data.dmi_window+1,
                                  frequency=context.frequency)
    if len(hist.index) < context.user_data.dmi_window+1:
