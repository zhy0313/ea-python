...
# 关键词：三重指数移动平均、降低交易频率。
# 方法：
# 1)结合单一EMA、双重EMA以及三重EMA，计算出三重指数的移动平均值；
# 2)当价格高于均值时买入，低于均值时卖出
...
def initialize(context):
    ...
    # 获取历史数据的长度
    context.user_data.team_window = 60
...
def handle_data(context):
    # 获取历史数据, 取后window根bar
    hist = context.data.get_price(context.security, count=context.user_data.team_window*4,
                                  frequency=context.frequency)
    if len(hist.index) < context.user_data.team_window*4:
    ...
    # 计算指标值
    tema = talib.TEMA(close, timeperiod=context.user_data.team_window)
