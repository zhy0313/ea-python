...
# 关键词：平均方向运动指标、趋势判断。
# 方法：
# 通过计算自价格达到近期最高值和最低值以来所经过的期间数，帮助预测价格趋势的变化。
...
def initialize(context):
    ...
    # 获取历史数据的长度
    context.user_data.aroon_window = 20
...
def handle_data(context):
    # 获取历史数据, 取后window根bar
    hist = context.data.get_price(context.security, count=context.user_data.aroon_window * 10,
                                  frequency=context.frequency)
    if len(hist.index) < context.user_data.aroon_window * 10:
    ...
    # 计算指标值
    aroon_down, aroon_up = talib.AROON(high, low, timeperiod=context.user_data.aroon_window)
    aroon_diff = aroon_up - aroon_down

    #  产生买入/卖出信号
    if aroon_diff[-1] > 0:
        if aroon_down[-1] < 50 <= aroon_down[-2] or aroon_up[-1] > 70 >= aroon_up[-2]:
            long_signal_triggered = True
    elif aroon_diff[-1] < 0:
        if aroon_up[-1] < 50 <= aroon_up[-2] or aroon_down[-1] > 70 >= aroon_down[-2]:
