...
def initialize(context):
    ...
    # 获取历史数据的长度
    context.user_data.bop_window = 10
...
def handle_data(context):
    ...
    hist = context.data.get_price(context.security, count=context.user_data.bop_window*10,
    ...
    if short_signal_triggered:
        if context.account.huobi_cny_btc >= HUOBI_CNY_BTC_MIN_ORDER_QUANTITY:
            ...
            context.order.sell_limit(context.security, quantity=str(context.account.huobi_cny_btc), price=str(hist_close[-1]*0.98))
        else:
    ...
    # 有买入信号，且持有现金，则市价单全仓买入
        ...
        if context.account.huobi_cny_cash >= HUOBI_CNY_BTC_MIN_ORDER_CASH_AMOUNT:
            ...
            context.order.buy_limit(context.security, quantity=str(context.account.huobi_cny_cash/hist_close[-1]*0.98), price=str(hist_close[-1]*1.02))
        else:
