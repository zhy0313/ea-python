...
def initialize(context):
    ...
    # 获取历史数据的长度
    context.user_data.cmo_window = 30
    context.user_data.over_buy = 70
    context.user_data.over_sell = -30
    # 至此initialize函数定义完毕。
...
def handle_data(context):
    # 获取历史数据, 取后window根bar
    hist = context.data.get_price(context.security, count=context.user_data.cmo_window+1,
                                  frequency=context.frequency)
    if len(hist.index) < context.user_data.cmo_window+1:
    ...
    # 计算AD值
    cmo = talib.CMO(hist_close, timeperiod=context.user_data.cmo_window)

    #  产生买入/卖出信号
    if cmo[-1] < context.user_data.over_sell:
        long_signal_triggered = True
    elif cmo[-1] > context.user_data.over_buy:
