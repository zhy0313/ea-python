...
def handle_data(context):
    ...
    # 根据策略调整仓位
    if cash_to_spent > HUOBI_CNY_ETH_MIN_ORDER_CASH_AMOUNT:
        #  市价单买入一定金额
        context.log.info("正在买入 %s" % context.security)
        ...
        #  计算需要卖出的数量，并已市价单卖出
        quantity = min(context.account.huobi_cny_eth, -1 * cash_to_spent / price)
        if quantity >= HUOBI_CNY_ETH_MIN_ORDER_QUANTITY:
