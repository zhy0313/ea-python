...
# 策略名称：ADOSC指标策略
# 关键词：Chaikin A/D Oscillatorreal线、震荡指标。
# 方法：
...
def initialize(context):
    ...
    # 获取历史数据的长度
    context.user_data.adosc_window = 14
    context.user_data.fast_period = 3
    context.user_data.slow_period = 10
...
def handle_data(context):
    ...
    hist = context.data.get_price(context.security, count=context.user_data.adosc_window*10,
                                  frequency=context.frequency)
    if len(hist.index) < context.user_data.adosc_window*10:
    ...
    # 计算AD值
    ad = talib.ADOSC(high, low, close, volume, fastperiod=context.user_data.fast_period, slowperiod=context.user_data.slow_period)
