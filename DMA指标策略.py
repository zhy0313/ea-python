...
# 关键词：平行线差指标、中短期指标。
# 方法：
# 1)计算两个不同时间频率的均线的差值，以及此差值的均值；
# 2)当差值大于均值时买入，小于均值时卖出
...
def initialize(context):
    ...
    # 获取历史数据的长度
    context.user_data.short_window = 5
    context.user_data.long_window = 20
    context.user_data.ama_window = 10
...
def handle_data(context):
    # 获取历史数据, 取后dma_window根bar
    hist = context.data.get_price(context.security, count=context.user_data.long_window+context.user_data.ama_window+1,
                                  frequency=context.frequency)
    if len(hist.index) < context.user_data.long_window+context.user_data.ama_window+1:
    ...
    # 计算DMA值
    sma_short = talib.SMA(close, timeperiod=context.user_data.short_window)
    sma_long = talib.SMA(close, timeperiod=context.user_data.long_window)
    dma = sma_short - sma_long
    ama = talib.SMA(dma, timeperiod=context.user_data.ama_window)
