...
def initialize(context):
    ...
    # 获取历史数据的长度
    context.user_data.roc_window = 14
...
def handle_data(context):
    # 获取历史数据, 取后window根bar
    hist = context.data.get_price(context.security, count=context.user_data.roc_window+1,
                                  frequency=context.frequency)
    if len(hist.index) < context.user_data.roc_window+1:
    ...
    # 计算AD值
    roc = talib.ROC(close, timeperiod=context.user_data.roc_window)
